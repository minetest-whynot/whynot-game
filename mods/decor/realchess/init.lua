...
-- https://github.com/Gerold55/minetest-laptop

-- Hacky:
-- (1) formspec and infotext overriden in init
-- (2) The app uses directly the laptop's nodemeta instead of the OS-layer
-- (3) Feedback trough chat - bad visible

if minetest.global_exists("laptop") then
	laptop.register_app("realchess", {
		app_name = "Realchess",
		app_icon = "chessboard_top.png",
		app_info = "A Chess game",

		formspec_func = function(app, mtos)
			local data = mtos.bdev:get_app_storage('ram', 'realchess')
			if not data.init_done then
				realchess.init(mtos.pos) --formspec and infotext overriden
				data.init_done = true
				mtos:swap_node() -- restore the infotext
			end

			return [[ bgcolor[#080808BB;true]
					background[3,1;8,8;chess_bg.png]
					button[6.1,8.8;2,2;new;New game]
					list[context;board;3,1;8,8;]
					listcolors[#00000000;#00000000;#00000000;#30434C;#FFF] ]]
		end,

		receive_fields_func = function(app, mtos, sender, fields)
			return realchess.fields(mtos.pos, _, fields, sender)
		end,
		allow_metadata_inventory_move = function(app, mtos, player, from_list, from_index, to_list, to_index, count)
			return realchess.move(mtos.pos, from_list, from_index, to_list, to_index, count, player)
		end,
		on_metadata_inventory_move = function(app, mtos, player, from_list, from_index, to_list, to_index, count)
			return realchess.on_move(mtos.pos, from_list, from_index)
		end,
